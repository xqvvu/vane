...
export const alertmanagerProviderParser: ProviderParser = {
  kind: "alertmanager",
  parse(input) {
...
export function parseAlertmanagerCompatibleProvider(
  ...
  const occurredAt = normalizeAlertmanagerDate(
    status === "resolved"
...
export function parseAlertmanagerProvider(input: ProviderParseInput): ProviderParseResult {
  return alertmanagerProviderParser.parse(input);
}
...
function normalizeAlertmanagerDate(value: unknown, fallback: string): string {
