...
import { createFeishuSign, feishuSender } from "#/feishu.ts";
import type { FeishuConfig } from "#/feishu.ts";
...
describe("feishu sender", () => {
  ...
  it("reports Feishu error codes as delivery failures", async () => {
    ...
    expect(result.error).toBe("Feishu returned code 19024");
  });
  ...
  it("registers Feishu in the default destination registry", () => {
    const sender = createDefaultDestinationRegistry().get("feishu");

    expect(sender.kind).toBe("feishu");
  });
});
